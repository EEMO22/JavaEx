...
//		stringMethodsEx();
	...
	private static void stringBufferEx() {
		...
		StringBuffer sb = new StringBuffer("This");	//	기본 버퍼(10칸), ()채워도 되고 말아도 되고
		sb.append(" is pencil");	//	append는 맨 뒤에 추가
		sb.insert(8, "my ");		//	insert는 중간에 삽입
		sb.replace(8, 10, "your");	//	replace는 치환
		System.out.println(sb);		//	최종 문자열 생성(toString() 이 호출 된 것)
		
		sb.setLength(10);	//	버퍼 크기를 강제 변경
		System.out.println(sb);
		
		String s = new StringBuffer("This")		//	메서드 체이닝
						.append(" is pencil")
						.insert(8, "my ")
						.replace(8, 10, "your")
						.toString();
	...
	private static void stringMethodsEx() {
		...
		//	화이트 스페이스(공백, \t, \r, \n) 제거
		String s2 = "           Hello        ";	//	정제되지 않은 데이터
		String s3 = ", Java";
		System.out.println(s2.trim() + s3);	//	trin() -> 앞 뒤 공백문자 제거
		
		//	구분자로 분절
//		String[] split = str.split(" ");	//	공백 문자를 기준, 문자열 분절
		String[] split = str.toUpperCase().replace(",", "").split(" ");	
			
		//	메서드 체이닝 -> 대문자로 바꾸고, 컴마를 없애고, 공백으로 분리 를 한 번에
		for (String word: split) {	//	split 안의 데이터를 꺼내어 word에 입력 반복
